<template>
  <b-nav-item-dropdown class="dropdown-cart mr-25" menu-class="dropdown-menu-media" right @show="fetchItems">
    <template #button-content>
      <feather-icon :badge="4" class="text-body" icon="ShoppingCartIcon" size="21" />
    </template>
    ...
    <li class="dropdown-menu-header">
      <div class="dropdown-header d-flex">
        ...
        <b-badge pill variant="light-primary">4 Items</b-badge>
      </div>
    </li>

    <!-- Cart Items -->
    <vue-perfect-scrollbar v-if="items.length" :settings="perfectScrollbarSettings" class="scrollable-container media-list scroll-area" tagname="li">
      <b-media v-for="item in items" :key="item.name">
        ...
        <div class="media-heading">
          <h6 class="cart-item-title">
            <b-link class="text-body">{{ item.name }}</b-link>
          </h6>
    ...
    <li v-if="items.length" class="dropdown-menu-footer">
      ...
      <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" block :to="{ name: 'apps-e-commerce-checkout' }">Checkout</b-button>
    </li>
