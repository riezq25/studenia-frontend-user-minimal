  ...
  <div>
    ...
    <b-card no-body class="mb-0">
      ...
      >
        ...
        <template #cell(pertanyaan)="data"><div v-html="data.item.pertanyaan"></div></template>
...
export default {
  ...
  setup() {
    ...
    const option = [{ title: 'Square' }, { title: 'Rectangle' }, { title: 'Rombo' }, { title: 'Romboid' }]
    ...
    const fetchSoal = async () => {
      ...
      fetchMapel()
      await repoSoal.get().then(function (response) {
    ...
    const editSoal = async () => {
      ...
      await repoSoal.update({
        ...
        pembahasan_text: form.value.pembahasan.text,
        pembahasan_video: form.value.pembahasan.video,
      }, form.value.id_mapel).then((response) => {
        listSoal.value.push(response.data.data)
    ...
    const deleteSoal = async (id) => {
      ...
      await repoSoal.delete(id).then((response) => {
        fetchSoal()
        showToast('Notifikasi', response.data.message, 'primary')
        resetModal()
    ...
    return {
      isError, isLoading, isSubmitting, listSoal,
      option,
      selected,
      content,
      tableColumns,
