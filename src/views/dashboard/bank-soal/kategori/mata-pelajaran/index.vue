  ...
  <div>
    ...
    <b-card no-body class="mb-0">
          ...
          <b-col cols="12" md="6">
            <div class="d-flex align-items-center justify-content-end">
              ...
              <b-button
                ...
                v-b-modal.tambah-mata-pelajaran
                variant="primary"
              >
        ...
        <b-form>
          ...
          <b-form-group>
            ...
            <v-select
              ...
              label="title"
              :options="listKategoriSoalMapped"
              :reduce="ks => ks.value"
            />
          </b-form-group>
        </b-form>
      ...
      <b-table
        ...
        striped
        hover
        empty-text="Tidak ada yang ditemukan"
      >
        <template #table-busy>
...
export default {
  ...
  setup() {
    ...
    const tableColumns = [
      ...
      {
        key: 'kategori_soal_id', label: 'Kategori Soal',
        formatter: (value) => {
    ...
    const fetchData = async () => {
      ...
      fetchKategoriSoal()
      await repoMapel.get().then((response) => {
        isLoading.value = false
        mataPelajaran.value = response.data.data
      })
        .catch((error) => {
          if (error.response) {
            showToast('Error', error.response.data.message, 'danger', 'AlaertTriangleIcon')
    ...
    const fetchKategoriSoal = async () => {
      isLoading.value = true
      await repoKategoriSoal.get().then((response) => {
        isLoading.value = false
    ...
    const deleteMapel = async (id) => {
      ...
      await repoMapel.delete(id).then((response) => {
        fetchData()
        showToast('Notifikasi', response.data.message, 'primary')
        resetModal()
    ...
    return {
      isError, isLoading, isSubmitting, mataPelajaran,
      tableColumns, form,
