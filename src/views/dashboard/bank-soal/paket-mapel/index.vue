<template>
  <div>
    <!-- Tambah To Soal -->
    <b-modal id="tambah-paket-soal" size="md" centered title="Tambah Paket" cancel-title="Close" ok-title="Simpan" cancel-variant="outline-secondary" @show="resetModal" @hidden="resetModal" @ok="submitPaket">
      <b-form>
        <b-form-group label="Kategori Mata Pelajaran" label-for="mapel">
          <v-select id="mapel" v-model="form.mapel_soal_id" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" label="title" :options="listMapelMapped" :reduce="mapel => mapel.value" />
        </b-form-group>
        ...
        <b-form-group label="File excel" label-for="fileExcel">
          <b-form-file id="fielExcel" placeholder="Choose a file or drop it here..." drop-placeholder="Drop file here..." accept=".xls, .xlsx" @change="handleFileUpload" />
        </b-form-group>
      </b-form>
    </b-modal>

    <!-- Edit To Soal -->
    <b-modal id="edit-paket-soal" size="md" centered title="Edit Paket" cancel-title="Close" ok-title="Ubah" cancel-variant="outline-secondary" @ok="editPaket">
      <b-form>
        <b-form-group label="Kategori Mata Pelajaran" label-for="mapel">
          <v-select id="mapel" v-model="form.mapel_soal_id" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" label="title" :options="listMapelMapped" :reduce="mapel => mapel.value" />
        </b-form-group>
    ...
    <!-- Delete Try Out -->
    <b-modal id="hapus-paket-soal" cancel-variant="outline-secondary" ok-title="Hapus" cancel-title="Close" centered title="Hapus Soal" @ok="deletePaket(form.id)">
      <b-form>
        <b-form-group>
    ...
    <b-card no-body class="mb-0">
      ...
      <b-table :busy="isLoading" ref="refSoalListTable" class="position-relative" :items="listPaket" responsive :fields="tableColumns" primary-key="id" show-empty striped hover empty-text="Tidak ada data ditemukan">
        <template #table-busy>
        ...
        <template #cell(actions)="data">
          <b-dropdown variant="link" no-caret :right="$store.state.appConfig.isRTL">
            ...
            <b-dropdown-item @click="getPaket(data.item.id)" :to="{name: 'manajemen-paket-detail', params:{id:data.item.id}}">
