<template>
  <b-card v-if="tableData" no-body class="card-company-table">
    <h4 class="px-2 py-1">Rangking Kamu</h4>
    <b-table :items="tableData" responsive :fields="fields" class="mb-0">
      ...
      <template #cell(company)="data">
        <div class="d-flex align-items-center">
          ...
          <div>
            <div class="font-weight-bolder">{{ data.item.title }}</div>
            <div class="font-small-2 text-muted">{{ data.item.subtitle }}</div>
          </div>
        </div>
      </template>
      ...
      <!-- revenue -->
      <template #cell(revenue)="data">{{ '$'+data.item.revenue }}</template>

      <!-- sales -->
      <template #cell(sales)="data">
        <div class="d-flex align-items-center">
          <span class="font-weight-bolder mr-1">{{ data.item.sales+'%' }}</span>
          <feather-icon :icon="data.item.loss ? 'TrendingDownIcon':'TrendingUpIcon'" :class="data.item.loss ? 'text-danger':'text-success'" />
        </div>
      </template>
    </b-table>
  </b-card>
