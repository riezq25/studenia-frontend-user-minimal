<template>
  <div>
    <!-- Tambah Try Out -->
    <b-modal id="tambah-try-out" cancel-variant="outline-secondary" ok-title="Tambah" cancel-title="Cancel" centered title="Tambah Try Out">
      <b-form>
        <b-form-group>
          <label for="nama-paket">Nama Paket:</label>
          <b-form-input id="nama-paket" type="text" placeholder="Nama Paket" />
        </b-form-group>
        <b-form-group>
          <label for="total-durasi">Total durasi:</label>
          <b-form-input id="total-durasi" type="text" placeholder="Total durasi" />
        </b-form-group>
      </b-form>
    </b-modal>

    <!-- Edit try out -->
    <b-modal id="edit-try-out" cancel-variant="outline-secondary" ok-title="edit" cancel-title="Cancel" centered title="Edit Try Out">
      <b-form>
        <b-form-group>
          <label for="nama-paket">Nama Paket:</label>
          <b-form-input id="nama-paket" type="text" placeholder="Nama Paket" />
    ...
    <!-- Delete Try Out -->
    <b-modal id="hapus-try-out" cancel-variant="outline-secondary" ok-title="Hapus" cancel-title="Close" centered title="Hapus Try Out">
      <b-form>
        <b-form-group>
          <p>Apakah anda akan menghapus Try Out ini?</p>
        </b-form-group>
      </b-form>
    </b-modal>
        ...
        <b-row>
          ...
          <b-col cols="12" md="6">
            <div class="d-flex align-items-center justify-content-end">
              ...
              <b-button variant="primary" v-b-modal.tambah-try-out>
                <span class="text-nowrap">Tambah Try Out</span>
              </b-button>
            </div>
          </b-col>
        </b-row>
      ...
      <b-table ref="refUserListTable" class="position-relative" :items="fetchUsers" responsive :fields="tableColumns" primary-key="id" :sort-by.sync="sortBy" show-empty empty-text="No matching records found" :sort-desc.sync="isSortDirDesc">
        ...
        <template #cell(id)="data">
          <b-media vertical-align="center">
            ...
            <b-link :to="{ name: 'admin/manajemen-try-out/premium/campuran/to-mapel', params: { id: data.item.id } }" class="font-weight-bold d-block text-nowrap">{{ data.item.fullName }}</b-link>
        ...
        <template #cell(actions)>
          <b-dropdown variant="link" no-caret :right="$store.state.appConfig.isRTL">
            ...
            <b-dropdown-item v-b-modal.edit-try-out>
              <feather-icon icon="EditIcon" />
              <span class="align-middle ml-50">Edit</span>
            </b-dropdown-item>

            <b-dropdown-item v-b-modal.hapus-try-out>
...
export default {
  ...
  setup() {
    ...
    const tableColumns = [
      ...
      { key: "nama_paket", sortable: true },
      { key: "total_durasi", sortable: true },
      { key: "actions" },
    ];
