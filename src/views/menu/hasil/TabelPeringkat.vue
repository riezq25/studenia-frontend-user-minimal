<template>
  <b-card-code title="Tabel Peringkat Kamu" no-body>
    <b-card-body>
      <div class="d-flex justify-content-between align-items-center flex-wrap">
        ...
        <div class="my-1">
          <h5 class="text-muted">Tabel ini berdasarkan Uji Coba Try Out (nama try out)</h5>
        </div>

        <!-- filter -->
        <b-form-group label-align-sm="left" label-size="sm" label-for="filterInput" class="mb-0">
          <b-input-group size="md">
            <b-form-input id="filterInput" v-model="filter" type="search" placeholder="Cari Nama" />
    ...
    <b-table striped hover responsive class="position-relative" :per-page="perPage" :current-page="currentPage" :items="items" :fields="fields" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered">
      <!-- <template #cell(status)="data">
        <b-badge :variant="status[1][data.value]">{{ status[0][data.value] }}</b-badge>
      </template>-->
    </b-table>
    ...
    <!-- <template #code>{{ codeKitchenSink }}</template> -->
  </b-card-code>
</template>
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "nilai_tka", label: "Nilai TKA", sortable: true },
        { key: "nilai_tps", label: "Nilai TPS", sortable: true },
        {
          key: "total_nilai",
          label: "Total Nilai",
          thClass: "text-center",
