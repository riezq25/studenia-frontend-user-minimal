  ...
  <b-card v-if="data" no-body class="card-revenue-budget">
    <b-row class="mx-0">
      <b-col md="8" class="revenue-report-wrapper">
        ...
        <!-- chart -->
        <vue-apex-charts id="revenue-report-chart" type="bar" height="230" :options="revenueReport.chartOptions" :series="data.revenueReport.series" />
      </b-col>

      <b-col md="4" class="budget-wrapper">
        <b-dropdown text="2020" size="sm" class="budget-dropdown" variant="outline-primary">
          <b-dropdown-item v-for="year in data.years" :key="year">{{ year }}</b-dropdown-item>
        </b-dropdown>
        ...
        <vue-apex-charts id="budget-chart" type="line" height="80" :options="budgetChart.options" :series="data.budgetChart.series" />

        <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary">Increase Budget</b-button>
      </b-col>
    </b-row>
  </b-card>
</template>
