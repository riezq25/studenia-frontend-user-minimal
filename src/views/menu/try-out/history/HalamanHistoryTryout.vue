    ...
    <div class="mb-2" v-if="!isLoading">
        <div v-if="tryout" class="card">
            <b-table
                ...
                hover
                responsive
                :items="tryout.pengerjaan_tryouts"
                :fields="tableColumns" :busy.sync="isLoading"
            >
                <template #cell(actions)="data">
                    <div class="flex flex-wrap justify-content-around">
                        <button class="btn btn-outline-success mr-1">Hasil</button>
                        <button
                            class="btn btn-outline-primary"
                            @click="redirectHalamanReview(data.item.id)"
                        >Review</button>
                    </div>
                </template>
            </b-table>
...
import { BTable, BButton, BModal, VBModal, BCardText, BRow, BCol } from "bootstrap-vue";
